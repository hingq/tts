...
* **异步任务、SSE 实时推送**：长任务（90 万字约 20–40 分钟）通过任务 ID + 状态查询 + SSE 实时事件流（Server-Sent Events）推送与文件下载解耦，避免反向代理超时并提供更好的客户端交互。
* **并发控制与磁盘保护**：通过配置 `MAX_CONCURRENT_JOBS` 限制全局运行中任务数，并在任务启动前强制校验磁盘剩余空间是否大于预估峰值的 1.2 倍，防止磁盘耗尽引发崩溃。
* **网络适应性（代理支持）**：支持通过 `TTS_PROXY` 等环境变量配置代理，保证在受限网络环境下也能稳定通过 WebSocket 调用微软 Edge-TTS。
...
* **可恢复性**：单机模式下基于 `state.json` 检查点实现断点续传，服务重启后通过自动扫描 `TMP_ROOT` 目录下的 `state.json` 重建内存状态并实现断点续传。
* **可读性与文档注释**：必须为项目内所有核心的逻辑块、较复杂的算法以及重要的公共函数和 API 编写清晰的中文注释与 JSDoc，确保代码逻辑清晰可见，降低长期协作或独立维护的理解成本。
...
* **代理支持**：支持加载环境变量 `TTS_PROXY`、`HTTPS_PROXY` 或 `HTTP_PROXY`，使用 `https-proxy-agent` 包装并代理底层 WebSocket 连接，确保能够稳定穿透代理。
* **参数映射**：
...
### 3.5.3 全局任务并发限制
除单个任务内部的并发池外，系统还在全局层面提供并发任务数量控制：
* **全局并发任务上限**：配置 `MAX_CONCURRENT_JOBS`（默认为 2），限制系统同时处于运行中（Running/Preprocessing/TTS/Mux/Validating）状态的 Job 数量。
* **溢出处理机制**：当达到上限后，新的创建任务请求（`POST /api/v1/audiobook/jobs`）会被立即拒绝，返回 `503 Service Unavailable`，防止单机内存、CPU 及磁盘 IO 过载。
...
* **响应**：
  ...
  * **磁盘空间不足 (`507 Insufficient Storage`)**：
    ```json
    {
      ...
      "message": "Insufficient disk space for processing this audiobook. Expected at least X MB, but only Y MB available."
    }
    ```
...
* **请求**：`GET /api/v1/audiobook/jobs/:jobId/events`
* **响应头**：
  ```
  HTTP/1.1 200 OK
  ...
  ```
* **推送事件**：
  * **进度更新 (`event: progress`)**：实时分发任务子阶段及 chunk 级别的合成与转码进度。
    ```json
    {
      "jobId": "8a1f0c9e-2c1a-4f8a-b5d7-6e3a9c5e2f10",
      "status": "running",
    ...
    ```
  * **状态完成或失败 (`event: status`)**：当任务结束时，发送最终状态及下载地址，然后服务端自动断开连接。
    ```json
    {
      "jobId": "8a1f0c9e-2c1a-4f8a-b5d7-6e3a9c5e2f10",
      "status": "done", // done | failed | canceled
      "downloadUrl": "/api/v1/audiobook/jobs/8a1f0c9e-2c1a-4f8a-b5d7-6e3a9c5e2f10/file",
      "error": null
    }
    ```
...
* **响应头**：
  ...
  Accept-Ranges: bytes
  ```
* 响应成功结束（`close` 事件）后，触发后台工作目录的清理动作。
...
| `MAX_CONCURRENT_JOBS` | `2` | `1 ~ 20` | 全局活跃任务最大限制（超过返回 503） |
...
| `TTS_PROXY` | `无` | 代理 URL (如 `http://127.0.0.1:7890`) | msedge-tts 的 WebSocket 连接代理 |
| `FFMPEG_PATH` | `ffmpeg` | 可执行文件路径 | 自定义 FFmpeg 路径，默认从系统 PATH 查找 |
| `FFPROBE_PATH` | `ffprobe` | 可执行文件路径 | 自定义 FFprobe 路径，默认从系统 PATH 查找 |
...
2. **灾难性错误（Fatal Error）**：
   ...
   - *前置磁盘空间预检*：任务启动前，基于 `fs.promises.statfs` 获取 `TMP_ROOT` 分区的可用空间。预估峰值空间估算公式为：
     $$\text{Expected Peak Space} = N_{\text{chunks}} \times (700\text{KB (raw MP3)} + 600\text{KB (standardized M4A)}) + \text{M4B size (approx. } N_{\text{chunks}} \times 600\text{KB)}$$
     即：每一千字（按单分片均值计）预留约 1.9MB。若可用空间低于 $\text{Expected Peak Space} \times 1.2$，则直接返回 `507` 错误。
...
4. **退出进程**：释放所有文件句柄后 `process.exit(0)`。

#### 5.3.3 服务重启状态重建与 GC 清理
1. **服务启动状态恢复**：服务每次启动时，会自动扫描 `TMP_ROOT` 目录，读取每个 Job 目录下的 `state.json`。若发现未完成（处于 pending/running）的任务，将在内存中重新初始化该 Job 实例及其对应的并发队列，自动读取 `state.json` 记录，跳过已生成的 M4A 片段，实现断点续传。
2. **定时垃圾回收 (GC)**：系统启动一个后台定时任务（每小时执行一次），扫描 `TMP_ROOT`。若某个 Job 目录的创建时间或其 `state.json` 的最后修改时间已超过 24 小时，则判定为过期未下载或废弃文件，异步递归删除该工作目录以释放磁盘。
...
项目选用 **Vitest** 现代化测试框架，对核心的无状态算法进行严格的单元测试与断言验证：
1. **文本预处理测试**：
   - 验证繁简转换映射的正确性。
   - 验证 SSML/HTML 标签剥离，防范 SSML 注入。
2. **章节匹配正则与校验测试**：
...
3. **字数统计与磁盘配额估算测试**：
   ...
   - 验证根据字数估算 Expected Peak Space 的边界公式计算。
